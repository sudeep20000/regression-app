...
def main(excel_path, independent_vars, dependent_var):
    ...
    unique_images = data['Img No'].unique()
    ...
    ax.set_title(f'3D Plot: Voltage vs. Time')
    ax.set_xlabel('Time(sec)')
    ax.set_ylabel('Voltage(volts)')
    ax.set_zlabel('C Rate')
    ax.legend()
    ...
    # Unique curve combinations (Env Temp and C Rate)
    unique_curves = filtered_data[['Img No', 'Env Temp']].drop_duplicates()
    ...
    for curve in unique_curves:
        curve_filter = (
            (filtered_data['Img No'] == curve[0]) &
            (filtered_data['Env Temp'] == curve[1])
        )
        ...
        with open(resultTxt_path, 'a') as f:
            f.write(f"\nResults for Img No {curve['Img No']} Temp {curve['Env Temp']} :\n")
        ...
        plt.title(f'Actual vs Predicted for Img No {curve["Img No"]} Temp {curve["Env Temp"]}')
        plt.legend()
        ...
        plot_filename = os.path.join(plot_folder, f"Image_{curve['Img No']}_Temp_{curve['Env Temp']}.png")
...
if __name__ == "__main__":
    ...
    main(excel_path, independent_vars, dependent_var)